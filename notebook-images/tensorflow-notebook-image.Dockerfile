...
RUN useradd --shell /bin/bash --uid $NB_UID \
...
EXPOSE 8888
ENTRYPOINT ["tini", "--"]
CMD jupyter notebook --notebook-dir=$NB_HOME --ip=0.0.0.0 --no-browser --allow-root --port=8888 --NotebookApp.token='' --NotebookApp.password='' --NotebookApp.allow_origin='*' --NotebookApp.base_url=$NB_PREFIX